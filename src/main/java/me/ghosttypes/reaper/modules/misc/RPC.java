...
public class RPC extends ReaperModule {
    ...
    private final ArrayList<String> rgb_names = new ArrayList<>(List.of("reaper", "reaper", "reaper", "reaper", "reaper", "reaper", "reaper"));
    ...
    public void onActivate() {
        checkMeteorRPC();
        DiscordIPC.start(957678651057520660L, null);
        rpc.setStart(System.currentTimeMillis() / 1000L);
        rgbTimer = MathUtil.intToTicks(rgbDelay.get());
        if (rgbLogo.get()) {
            rpc.setLargeImage("reaper", "reaper+ " + Reaper.VERSION);
            rgb_i++;
        } else {
    ...
    private String getImgKey() {
        ...
        switch (logo.get()) {
            case Default -> imgKey = "reaper";
            case Legacy -> imgKey = "reaper";
            case Letter -> imgKey = "reaper";
            case Red -> imgKey = "reaper";
            case Orange -> imgKey = "reaper";
            case Yellow -> imgKey = "reaper";
            case Green -> imgKey = "reaper";
            case Blue -> imgKey = "reaper";
            case Purple -> imgKey = "reaper";
            case Pink -> imgKey = "reaper";
            case White -> imgKey = "reaper";
        }
        return imgKey;
    }
