...
import static me.ghosttypes.reaper.util.helper.TimerUtils.*;
...
public class StrafePlus extends Module {
    ...
    public StrafePlus() {
        super(ML.R, "strafe+", "Increase speed and control.");
    }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (WorldUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && webbedPause.get() == WebbedPause.Always) return;
        if (WorldUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;
