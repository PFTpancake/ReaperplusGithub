...
public class Nametags extends ReaperModule {
    ...
    private final Setting<List<Enchantment>> ignoredEnchantments = sgPlayers.add(new EnchantmentListSetting.Builder().name("ignored-enchantments").description("The enchantments that aren't shown on nametags.").visible(displayItemEnchants::get).build());
    ...
    private void renderNametagPlayer(PlayerEntity player) {
        ...
        boolean isDev = Interactions.isDev(player.getEntityName());
        if (isDev) width += devWidth;
        boolean isbeta = Interactions.isBeta(player.getEntityName());
        if (isbeta) width += betawith;
        ...
        if (isDev) hX = text.render(devText, hX, hY, RED, true);
        if (isbeta) hX = text.render(betaText, hX, hY, GOLD, true);
