...
public class PistonAlert extends ReaperModule {
    ...
    public PistonAlert() {
        super(ML.M, "PistonAlert", "alerts you about nearby Pistonfags");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        for (Entity e : mc.world.getEntities()) {
            if (e instanceof PlayerEntity player && player != mc.player && mc.player.distanceTo(player) <= range.get()) {
                if (player.getMainHandStack().getItem().equals(Items.STICKY_PISTON) || player.getOffHandStack().getItem().equals(Items.STICKY_PISTON)) {
                    if (!bedFags.contains(player)) { // bedfag detection
                        bedFags.add(player);
                        warning(player.getEntityName() + " is Pistonfagging.");
                    }
                    ...
                    info(player.getEntityName() + " is no longer Pistonfaging.");
                    bedFags.remove(player); // remove once they aren't holding a bed
                }
                if (bedFags.contains(player)) {
                    if (player.getMainHandStack().getItem().equals(Items.PISTON) || player.getOffHandStack().getItem().equals(Items.PISTON)) {
                        if (!craftFags.contains(player)) { // crafting detection
                            craftFags.add(player);
                            warning(player.getEntityName() + " is Pistonfaging.");
                        }
                    }
                } else if (craftFags.contains(player)) {
                    info(player.getEntityName() + " isPistonfagging.");
