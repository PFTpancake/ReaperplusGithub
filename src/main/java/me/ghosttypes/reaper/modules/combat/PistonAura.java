...
public class PistonAura extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        FindItemResult anchor = InvUtils.findInHotbar(Items.PISTON);
        FindItemResult glowStone = InvUtils.findInHotbar(Items.END_CRYSTAL);
    ...
    private BlockPos findPlacePos(BlockPos targetPos) {
        switch (placePositions.get()) {
            case Above:
                ...
                else if (isValidBreak(targetPos.add(2, 1, 0))) return targetPos.add(1, 0, 0);
                else if (isValidBreak(targetPos.add(-2, 1, 0))) return targetPos.add(-1, 0, 0);
                else if (isValidBreak(targetPos.add(0, 1, 2))) return targetPos.add(0, 0, 1);
                else if (isValidBreak(targetPos.add(0, 1, -2))) return targetPos.add(0, 0, -1);
                break;
        }
        return null;
    ...
    private boolean isValidBreak(BlockPos pos) {
        return mc.world.getBlockState(pos).getBlock() == Blocks.PISTON && Math.sqrt(mc.player.getBlockPos().getSquaredDistance(pos)) <= breakRange.get() && getDamageBreak(pos);
    }

    private void breakAnchor(BlockPos pos, FindItemResult anchor, FindItemResult glowStone) {
        if (pos == null || mc.world.getBlockState(pos).getBlock() != Blocks.PISTON) return;
