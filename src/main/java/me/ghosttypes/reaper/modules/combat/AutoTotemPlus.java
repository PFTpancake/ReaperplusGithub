...
public class AutoTotemPlus extends Module {
    ...
    public AutoTotemPlus() {
        super(ML.R, "AutoTotemPlus", "Automatically replenishes your totems.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (totems <= 0) locked = false;
            ...
            boolean low = mc.player.getHealth() + mc.player.getAbsorptionAmount()  <= health.get();
            boolean ely = mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem() == Items.ELYTRA && mc.player.isFallFlying();

            locked = mode.get() == Mode.Strict && (low || ely);
